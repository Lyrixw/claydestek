...
client.on("interactionCreate", async interaction => {      
    if (interaction.values == 'ic') {
        ...
        const ticketolusturdun = new EmbedBuilder()
                  .setAuthor({name: `ᴄʟᴀʏ - ᴅᴇsᴛᴇᴋ sɪsᴛᴇᴍɪ`, iconURL: interaction.user.displayAvatarURL({ dynamic: true }) })
...
client.on("interactionCreate", async interaction => {
    ...
    if (interaction.values == 'secimiptal') {
        ...
        .setTimestamp()
        await interaction.reply({embeds: [ᴄʟᴀʏsecim], ephemeral: true})
    }
